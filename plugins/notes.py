...
from Alex.mongo.notesdb import *
from utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
...
async def _save(client, message):
    ...
    if message.reply_to_message and not len(message.command) >= 2:
        return await message.reply_text("ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ɢɪᴠᴇ ᴛʜᴇ ɴᴏᴛᴇ ᴀ ɴᴀᴍᴇ!")

    if not message.reply_to_message and not len(message.command) >= 3:
        return await message.reply_text("ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ɢɪᴠᴇ ᴛʜᴇ ɴᴏᴛᴇ sᴏᴍᴇ ᴄᴏɴᴛᴇɴᴛ!")
    ...
    await message.reply_text(f"ɪ'ᴠᴇ sᴀᴠᴇᴅ ɴᴏᴛᴇ <code>{NoteName}</code> ɪɴ {chat_title}.")
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        if (
            ...
            await message.reply(
                "ɴᴏᴡ ɪ ᴡɪʟʟ sᴇɴᴅ ᴀ ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ ᴄʜᴀᴛ ᴡɪᴛʜ ᴀ ʙᴜᴛᴛᴏɴ ʀᴇᴅɪʀᴇᴄᴛɪɴɢ ᴛᴏ <b>ᴘᴍ</b>, ᴡʜᴇʀᴇ ᴛʜᴇ ᴜsᴇʀ ᴡɪʟʟ ʀᴇᴄᴇɪᴠᴇ ᴛʜᴇ ɴᴏᴛᴇ.",
                quote=True
            )
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. Alex will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
                "ʏᴏᴜʀ ɴᴏᴛᴇs ᴀʀᴇ ᴄᴜʀʀᴇɴᴛʟʏ ʙᴇɪɴɢ sᴇɴᴛ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ.",
                quote=True
            )
...
async def Clear_Note(client, message):
    ...
    if not (
        ...
        await message.reply(
            "ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ɢɪᴠᴇ ᴛʜᴇ ɴᴏᴛᴇ ᴀ ɴᴀᴍᴇ!",
            quote=True
        )
        return
    ...
    if await isNoteExist(chat_id, note_name):
        ...
        await message.reply(
            f"ɪ'ᴠᴇ ʀᴇᴍᴏᴠᴇ ɴᴏᴛᴇ <code>{note_name}</code>!.",
            quote=True
        )
    else:
        await message.reply(
            "ʏᴏᴜ ʜᴀᴠᴇɴ'ᴛ sᴀᴠᴇᴅ ᴀ ɴᴏᴛᴇ ᴡɪᴛʜ ᴛʜɪs ɴᴀᴍᴇ ʏᴇᴛ!",
            quote=True
        )
...
async def ClearAll_Note(client, message):
    ...
    await message.reply(
        f"ᴀʀᴇ ʏᴏᴜ sᴜʀᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʟᴇᴀʀ <b>ᴀʟʟ</b> ɴᴏᴛᴇs ɪɴ {chat_title}? ᴛʜɪs ᴀᴄᴛɪᴏɴ ɪs ɪʀʀᴇᴠᴇʀsɪʙʟᴇ.",
...
async def ClearAllCallback(client, callback_query: CallbackQuery):
    ...
    if owner_id == user_id:
        if query_data == 'clear':
            ...
            await callback_query.edit_message_text("ᴅᴇʟᴇᴛᴇᴅ ᴀʟʟ ᴄʜᴀᴛ ɴᴏᴛᴇs.") 
            return

        elif query_data == 'cancel':
            await callback_query.edit_message_text("Cancelled.")
    else:
        await callback_query.answer("ᴏɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ ᴇxᴇᴄᴜᴛᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ!")
...
async def Notes(client, message):
    ...
    if (
        ...
        for notes in Notes_list:
            NoteName = f" • <code>#{notes}</code>\n"
            NoteHeader += NoteName
        await message.reply(
            (
                f"{NoteHeader}\n"
                "ʏᴏᴜ ᴄᴀɴ ʀᴇᴛʀɪᴇᴠᴇ ᴛʜᴇsᴇ ɴᴏᴛᴇs ʙʏ ᴜsɪɴɢ <code>/get notename</code>, ᴏʀ <code>#notename</code>"
            ),
            quote=True
        )
